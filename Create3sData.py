...
def analyze_audio(file_path, label, i, duration=30):
    ...
    for start in range(max_start, max_start + duration * sr, 3 * sr):
        ...
        segment_features = [chroma_stft_mean.mean(), chroma_stft_var.mean(),
                            rms_mean.mean(), rms_var.mean(), spectral_centroid_mean.mean(),
                            spectral_centroid_var.mean(), spectral_bandwidth_mean.mean(),
                            spectral_bandwidth_var.mean(), rolloff_mean.mean(), rolloff_var.mean(),
