...
def analyze_audio(file_path, duration=30):
    # 음원 파일 로드
    ...
    # 특징들을 리스트로 반환
    return [os.path.basename(file_path), len(y), chroma_stft_mean.mean(), chroma_stft_var.mean(),
            ...
            harmony_mean, harmony_var, perceptr_mean, perceptr_var, tempo] + mfcc_means.tolist() + mfcc_vars.tolist()
