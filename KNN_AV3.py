...
for label in unique_labels:
    ...
    z_scores = np.abs((X_label - X_label.mean()) / X_label.std())  # Z 점수 계산

    outliers = z_scores > threshold  # 이상치 여부 확인

    # 이상치를 평균으로 대체
    X_label[outliers] = X_label.mean()  # 이상치를 평균 값으로 대체
    print(np.array_equal(X[y == label], X_label))
    # 수정된 데이터셋을 다시 원래의 데이터에 할당
    X[y == label] = X_label
...
X_train_O, X_test_O, y_train_O, y_test_O = train_test_split(Z,D, test_size=0.2,random_state=random_number)
...
#     # 이상치를 평균으로 대체
#     X_label[outliers] = X_label.mean()  # 이상치를 평균 값으로 대체
#     print(np.array_equal(X_train[y_train == label], X_label))
#     # 수정된 데이터셋을 다시 원래의 데이터에 할당
#     X_train[y_train == label] = X_label
